...
namespace nanoFramework.Tools.DebugEngine
{
    ...
    class AD7Thread : IDebugThread2, IDebugThread100
    {
        ...
        const string ThreadNameString = "nanoFramework debug engine Thread";
        ...
        // For the nanoFramework debug engine, enumerating the stack frames requires walking the callstack in the debuggee for this thread
        ...
        // Get the name of the thread. For the nanoFramework debug engine, the name of the thread is always "nanoFramework debug engine Thread"
        ...
        int IDebugThread100.GetThreadProperties100(uint dwFields, THREADPROPERTIES100[] props)
        {
            ...
            if (hRes == Constants.S_OK && dwFields != (uint)dwFields90)
            {
                if ((dwFields & (uint)enum_THREADPROPERTY_FIELDS100.TPF100_DISPLAY_NAME) != 0)
                {
                    ...
                    props[0].bstrDisplayName = "<thread name from nanoFramework debug engine>";
