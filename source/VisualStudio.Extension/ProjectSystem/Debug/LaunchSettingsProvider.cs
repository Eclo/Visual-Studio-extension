...
namespace nanoFramework.Tools.VisualStudio.Extension
{
    ...
    internal class LaunchSettingsProvider : OnceInitializedOnceDisposed, ILaunchSettingsProvider2
    {
        ...
        // TODO protected SimpleFileWatcher FileWatcher { get; set; }
        ...
        // TODO public ITaskDelayScheduler FileChangeScheduler { get; protected set; }
        ...
        protected async Task<LaunchSettingsData> ReadSettingsFileFromDiskAsync()
        {
            ...
            {
                // TODO string err = string.Format(Resources.JsonErrorReadingLaunchSettings, readerEx.Message);
                //LogError(err, fileName, readerEx.LineNumber, readerEx.LinePosition, false);
                throw;
            }
            catch (JsonException jsonEx)
            {
                // TODO string err = string.Format(Resources.JsonErrorReadingLaunchSettings, jsonEx.Message);
                //LogError(err, fileName, -1, -1, false);
                throw;
            }
            catch (Exception ex)
            {
                // TODO string err = string.Format(Resources.ErrorReadingLaunchSettings, fileName, ex.Message);
                //LogError(err, false);
                throw;
            }
        }
        ...
        private List<LaunchProfileData> FixupProfilesAndLogErrors(Dictionary<string, LaunchProfileData> profilesData)
        {
            ...
            {
                // TODO LogError(Resources.ProfileMissingName, false);
            }
        ...
        protected async Task SaveSettingsToDiskAsync(ILaunchSettings newSettings)
        {
            ...
            {
                // TODO string err = string.Format(Resources.ErrorWritingDebugSettings, fileName, ex.Message);
                //LogError(err, false);
                throw;
            }
            finally
        ...
        /// </summary>
        protected async void LaunchSettingsFile_Changed(object sender, FileSystemEventArgs e)
        {
...
#pragma warning restore CS0618
        ...
        public async Task<ILaunchSettings> GetSnapshotThrowIfErrors()
        {
            ...
            if (currentSettings == null || (currentSettings.Profiles.Count == 1 && string.Equals(currentSettings.Profiles[0].CommandName, ErrorProfileCommandName, StringComparison.Ordinal)))
            {
                ...
                // TODO throw new Exception(string.Format(Resources.JsonErrorNeedToBeCorrected, fileName));
            }
